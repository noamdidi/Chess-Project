...
Pawn::Pawn(Board* board, const char team, const int row, const int column)
{
...
char Pawn::IsValidMove(Tile* src, Tile* dst)
{
	/*******************************************************************\
	...
	{
		return '6'; //Check if the requested move is valid for the pawn. 
	}
	...
	// No need to check if there are pieces in between the src and the dst
	
	if (this->_board->GetLayout()[dst->GetRow()][dst->GetColumn()]->GetType() == '#') // making sure a pawn won't be able to eat in a straight line
	{
...
bool Pawn::canIMoveLikeThis(Tile* dst)
{
	return ((dst->GetColumn() == this->_position->GetColumn()) && ((abs(dst->GetRow() - this->_position->GetRow()) == 1) || (this->_isPlayedYet == false && (abs(dst->GetRow() - this->_position->GetRow()) == 2))));
}
