...
Rook::Rook(Board* board, const char team, const int row, const int column)
{
	this->_team = team; 
...
char Rook::IsValidMove(Tile* src, Tile* dst)
{
	/*******************************************************************\
	...
	{
		return '6'; //Check if the requested move is valid for the rook. 
	}
...
bool Rook::canIMoveLikeThis(Tile* dst)
{
	return (((this->_position->GetRow() == dst->GetRow()) && (this->_position->GetColumn() != dst->GetColumn())) || ((this->_position->GetRow() != dst->GetRow()) && (this->_position->GetColumn() == dst->GetColumn())));
}
